...
.get(
    ...
    async (c) => {
      ...
      let response: { nextCursor: string | null; users: UserType[] } | null = null;
  
      try {
        const cachedData = await cache.get<{ nextCursor: string | null; users: UserType[] } | null>(cacheKey);
  
        if (cachedData && cachedData.users) {
          response = { nextCursor: cachedData.nextCursor, users: cachedData.users };
