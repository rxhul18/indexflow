...
.post("/:id", async (c) => {
  try {
    ...
    const isServer = serversData.configs.find((server: ConfigType) => server.server_id === svID);
    if(!isServer) {
        return c.json({
            success: false,
    ...
    if(key && isServer) {
        ...
        const serversData = await servers.json();
        const isOwner = serversData.servers.find((server: ServerType) => server.id === isServer.server_id).owner_id === key;
    ...
    const filteredQuestions = data.questions.filter((question: QuestionType) => question.server_id === key);
