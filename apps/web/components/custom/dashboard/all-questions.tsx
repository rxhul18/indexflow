...
export default function AllQuestions({tagName}:{
    ...
    const getSortedQuestions = (tag: string, sort: string) => {
        return [...questions].sort((a, b) => {
            ...
            if (sort === "newest") return compareDesc(new Date(a.createdAt), new Date(b.createdAt));
    ...
    const handleFilterChange = (value: string) => {
        router.push(`?sort=${value}${tagFil ? `&tag=${tagFil}` : ""}`, { scroll: false });
    };
    ...
    return (
        <div className="flex-1 relative h-[calc(100vh-120px)] overflow-x-hidden">
            ...
            <div className="space-y-4 relative overflow-y-auto h-full overflow-x-hidden [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
                ...
                {loading && <div className="justify-center items-center py-4 flex-1 flex">
